...
class DoubleLinkedList:
    ...
    def __init__(self) -> None:
        # 초기화
        self.head = self.current = Node()       # 더미노드 생성
        self.no = 0
    ...
    def prev(self) -> bool:
        # 주목 노드를 한 칸 앞으로 이동
        if self.is_empty() or self.currnet.prev is self.head:
            return False
        self.currnet = self.currnet.prev
        return True
    ...
    def add_last(self, data: Any) -> None:
        # 맨 뒤에 노드 삽입
        self.currnet = self.head.prev       # 꼬리 노드 head.prev의 바로 뒤에 삽입 
        self.add(data) 
