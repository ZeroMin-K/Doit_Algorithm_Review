...
class ArrayLinkedList:
    ...
    def get_insert_index(self):
        ...
        if self.deleted == Null:                # 삭제 레코드는 존재하지 않음
            ...
            rec = self.deleted
            self.deleted = self.n[rec].dnext    # 프리리스트에서 맨 앞 rec를 꺼내기
            return rec 
    ...
    def remove(self, p:int) -> None:
        ...
        if self.head != Null:
            if p == self.head:
                self.remove_first()                     # p가 머리 노드이면 머리 노드 삭제
            else:
                ptr = self.head
    ...
    def print_current_node(self) -> None:
        # 주목 노드 출력
        if self.currnet == Null:
            print('주목 노드가 없습니다')
        else:
            print(self.n[self.currnet].data)
